...
constexpr size_t LOCAL_SSID_MAX_LEN = 32;
...
size_t buildBeaconFrame(uint8_t *buf, const String &ssid, const uint8_t mac[6], uint8_t channel) {
    if (ssid.length() == 0 || ssid.length() > LOCAL_SSID_MAX_LEN) {
        Serial.printf("Warning: SSID length %d invalid, truncating to %d\n", ssid.length(), LOCAL_SSID_MAX_LEN);
    }
    ...
    uint8_t ssidLen = (uint8_t)min((size_t)LOCAL_SSID_MAX_LEN, ssid.length());
    buf[37] = ssidLen;
